...
void setup() {
  ...
  Serial.println("--- SENSOR TESTING RIG ---");
...
void loop() {
  if (selectedMode == 1) {
    ...
    Serial.println("Full cycle finished. Restarting in 5 seconds...");
    delay(5000);

  } else if (selectedMode == 2) {
    // --- ONLY HYSTERESIS TEST ---
    runHysteresisTest();

    Serial.println("Hysteresis cycle finished. Repeating in 5 seconds...");
    delay(5000);
  }
}

void runHysteresisTest() {
  Serial.println("--- PHASE 5: Hysteresis Sweep Test (Material Memory) ---");
  ...
  // Sweep Up (Forward direction - Bending)
  ...
  // Sweep Down (Backward direction - Straightening)
